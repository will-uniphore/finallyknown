...
var require_bindings = __commonJS({
  "../../node_modules/.pnpm/bindings@1.5.0/node_modules/bindings/bindings.js"(exports, module) {
    ...
    function bindings(opts) {
      ...
      Object.keys(defaults2).map(function(i3) {
        if (!(i3 in opts)) opts[i3] = defaults2[i3];
      });
      if (!opts.module_root) {
...
function daysSince(timestamp, now = /* @__PURE__ */ new Date()) {
...
function computeObservationConfidence(timesObserved, lastSeenAt, now = /* @__PURE__ */ new Date()) {
...
function observationCountForConfidence(confidence, elapsedDays) {
...
function parseSupportingNodes(raw) {
  try {
...
function stringifySupportingNodes(nodeIds) {
...
var KnownDB = class {
  ...
  findNodeByTypeAndText(_type, text) {
  ...
  reconfirmNodeObservation(id, updates) {
  ...
  applyContradictionPenalty(id, factor = 0.7) {
    const existing = this.getNode(id);
    if (!existing) {
      return void 0;
    }
  ...
  getNodesWithEmbeddings() {
    return this.db.prepare("SELECT * FROM nodes WHERE embedding IS NOT NULL AND confidence > 0.1").all();
  }
  removeNodeFromInsightSupport(nodeId) {
  ...
  deleteNodesBelow(confidenceThreshold) {
    const ids = this.db.prepare("SELECT id FROM nodes WHERE confidence < ?").all(confidenceThreshold);
    const tx = this.db.transaction(() => {
      for (const { id } of ids) {
        this.deleteNode(id);
      }
    ...
    this.db.prepare(
      `UPDATE edges
         SET confidence = ?,
             ...
             text = COALESCE(?, text)
         WHERE id = ?`
    ).run(Math.max(existing.confidence, edge.confidence ?? 1), edge.source ?? existing.source, edge.text ?? existing.text, existing.id);
  ...
  getEdgesFrom(nodeId) {
    return this.db.prepare("SELECT * FROM edges WHERE source_id = ?").all(nodeId);
  }
  getEdgesTo(nodeId) {
    return this.db.prepare("SELECT * FROM edges WHERE target_id = ?").all(nodeId);
  }
  getEdgesForNode(nodeId) {
    return this.db.prepare("SELECT * FROM edges WHERE source_id = ? OR target_id = ?").all(nodeId, nodeId);
  }
  getEdgesBetweenNodes(nodeIds) {
    if (nodeIds.length === 0) {
      return [];
    }
  ...
  mergeNodeInto(keepId, removeId) {
    ...
    const tx = this.db.transaction(() => {
      const insightMap = /* @__PURE__ */ new Map();
  ...
  updateInsightEmbedding(id, embedding) {
    this.db.prepare("UPDATE insights SET embedding = ? WHERE id = ?").run(embedding, id);
  }
  updateInsightSupportingNodes(id, nodeIds) {
  ...
  getInsight(id) {
    return this.db.prepare("SELECT * FROM insights WHERE id = ?").get(id);
  }
  getAllInsights() {
    return this.db.prepare("SELECT * FROM insights").all();
  }
  getInsightsWithEmbeddings() {
    return this.db.prepare("SELECT * FROM insights WHERE embedding IS NOT NULL").all();
  }
  getInsightsReferencingNode(nodeId) {
    return this.getAllInsights().filter((insight) => parseSupportingNodes(insight.supporting_nodes).includes(nodeId));
  }
  getTopInsights(limit2 = 10) {
    return this.db.prepare("SELECT * FROM insights ORDER BY confidence DESC LIMIT ?").all(limit2);
  }
  deleteDeadInsights() {
  ...
  // --- Graph Traversal ---
  expandViaEdges(nodeIds, depth = 1) {
    if (nodeIds.length === 0) {
      return [];
    }
    const placeholders = nodeIds.map(() => "?").join(",");
  ...
  getStats() {
    ...
    const insightCount = this.db.prepare("SELECT COUNT(*) AS c FROM insights").get().c;
...
function getOpenAIClient(config) {
  ...
  if (!client || clientApiKey !== config.openaiApiKey) {
    client = new openai_default({ apiKey: config.openaiApiKey });
...
async function generateEmbeddings(texts, config) {
  ...
  return response.data.sort((a2, b2) => a2.index - b2.index).map((item) => Buffer.from(new Float32Array(item.embedding).buffer));
}
function bufferToVector(buf) {
  return new Float32Array(buf.buffer, buf.byteOffset, buf.byteLength / Float32Array.BYTES_PER_ELEMENT);
}
function cosineSimilarity(a2, b2) {
  const va = bufferToVector(a2);
  const vb = bufferToVector(b2);
...
function semanticSearch(query, items, limit2 = 20) {
  return items.filter((item) => item.embedding !== null).map((item) => ({
    ...item,
    similarity: cosineSimilarity(query, item.embedding)
  })).sort((a2, b2) => b2.similarity - a2.similarity).slice(0, limit2);
}
function averageEmbedding(nodes) {
  ...
  for (const vector of vectors) {
    for (let i2 = 0; i2 < dim; i2 += 1) {
      avg[i2] += vector[i2];
    }
  }
  for (let i2 = 0; i2 < dim; i2 += 1) {
    avg[i2] /= vectors.length;
  }
  return Buffer.from(avg.buffer);
}
function getCategoryNodes(db, category, clusterSize) {
  return db.getNodesByType(category).slice(0, clusterSize);
}
function maximallyDistantClusters(db, clusterSize = 5) {
  const categories = db.getDistinctNodeTypes().map((category) => ({
    category,
    nodes: getCategoryNodes(db, category, clusterSize)
  })).filter((entry) => entry.nodes.length > 0).map((entry) => ({
    ...entry,
  ...
  let bestPair = null;
  for (let i2 = 0; i2 < categories.length; i2 += 1) {
    for (let j2 = i2 + 1; j2 < categories.length; j2 += 1) {
      const left = categories[i2];
      const right = categories[j2];
...
function shouldSurfaceInsight(insight) {
  return insight.times_rediscovered >= 2 && insight.confidence >= 0.6;
}
async function storeOrStrengthenInsight(db, config, text, supportingNodeIds, similarityThreshold = 0.9) {
...
var INGEST_SYSTEM = `You extract TRAIT CODES for "Known," a brain-like user understanding system.
...
var INGEST_USER = (sessionText) => `Extract pattern-level trait codes from this session.

${sessionText}`;
var INGEST_FACT_SYSTEM = `You extract SPECIFIC PERSONAL FACTS for "Known," a brain-like user understanding system.
...
- Return valid JSON only`;
var INGEST_FACT_USER = (sessionText) => `Extract specific personal facts from this session.

${sessionText}`;
var CONTRADICTION_SYSTEM = `You compare two trait codes about the same user.
...
var CONTRADICTION_USER = (existingTrait, newTrait) => `Existing trait code: ${existingTrait}
New trait code: ${newTrait}

Do these describe the same trait, a contradiction, or different nearby patterns?`;
var MIN_SESSION_CHARS = 500;
var DEDUP_SIMILARITY_THRESHOLD = 0.8;
var MAX_CONTRADICTION_CHECKS = 3;
var FACT_TYPE_PREFIX = "fact:";
function normalizeNodeType(type) {
  ...
  const normalized = rawType.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
  if (isFact) {
    return `${FACT_TYPE_PREFIX}${normalized || "detail"}`;
  }
  return normalized || "trait";
}
function isFactNodeType(type) {
  return type.startsWith(FACT_TYPE_PREFIX);
}
function isGenericNodeType(type) {
  return type === "trait" || type === `${FACT_TYPE_PREFIX}detail`;
}
function choosePreferredNodeType(existingType, incomingType) {
...
function hasUserMessages(sessionText) {
...
async function judgeObservationRelationship(config, existing, candidate) {
  const openai = getOpenAIClient(config);
...
var MAX_CHUNK_CHARS = 3e4;
function chunkText(text, maxChars) {
  if (text.length <= maxChars) return [text];
  const chunks = [];
  for (let i2 = 0; i2 < text.length; i2 += maxChars) {
    chunks.push(text.slice(i2, i2 + maxChars));
  }
  return chunks;
}
async function ingest(db, sessionText, config, sessionId) {
...
async function ingestChunk(db, sessionText, config, sessionId) {
  const openai = getOpenAIClient(config);
  ...
  const textToNodeId = /* @__PURE__ */ new Map();
  let nodesCreated = 0;
...
var THINK_SYSTEM = `You are the conscious reasoning engine for "Known," a brain-like user understanding system.
...
var THINK_USER = (question, nodes, edges, insights, agentContext) => {
  let prompt = `## Question
${question}
...
var ACTIVATION_DECAY_PER_HOP = 0.5;
...
function parseThinkResult(content) {
  try {
    const parsed = JSON.parse(content);
    return {
...
function activateNodes(db, seeds) {
...
async function think(db, question, config, agentContext) {
  const openai = getOpenAIClient(config);
  const queryEmbedding = await generateEmbedding(question, config);
  const allNodesWithEmb = db.getNodesWithEmbeddings();
  const relevantNodes = allNodesWithEmb.length <= 30 ? allNodesWithEmb.map((n2) => ({ ...n2, similarity: 1 })) : semanticSearch(queryEmbedding, allNodesWithEmb, 20);
  const activatedNodes = activateNodes(db, relevantNodes).slice(0, 25);
  const surfacedInsights = semanticSearch(queryEmbedding, db.getInsightsWithEmbeddings(), 10).filter((insight) => shouldSurfaceInsight(insight)).slice(0, 5);
...
var DISCOVER_SYSTEM = `You are the subconscious reasoning process for "Known," a brain-like user understanding system.
...
var DISCOVER_USER = (clusterA, clusterB, categoryA, categoryB) => {
...
function parseDiscoverResult(content) {
  try {
    return JSON.parse(content);
  } catch {
    return { found: false };
...
async function discover(db, config) {
  ...
  const supportingNodeIds = result.supporting_node_ids?.filter(
...
// src/index.ts
var DEFAULT_DISCOVER_INTERVAL_HOURS = 4;
var DEFAULT_MAINTAIN_INTERVAL_HOURS = 24;
var KNOWN_CONTEXT_PREAMBLE = "Known context about the user. Treat this as background guidance, not ground truth, and only use details that help with the current conversation.";
...
function expandPath(api, rawPath) {
  if (!rawPath) {
    return void 0;
  }
  if (rawPath === "~") {
  ...
  if (rawPath.startsWith("~/")) {
    return path.join(homedir2(), rawPath.slice(2));
  }
...
function resolveSettings(api) {
  ...
  return {
    ...
    discoverIntervalMs: asNonNegativeNumber(pluginConfig.discoverIntervalHours, DEFAULT_DISCOVER_INTERVAL_HOURS) * 60 * 60 * 1e3,
    maintainIntervalMs: asNonNegativeNumber(pluginConfig.maintainIntervalHours, DEFAULT_MAINTAIN_INTERVAL_HOURS) * 60 * 60 * 1e3
  };
}
function openDatabase(api) {
...
async function withDatabase(api, action) {
...
function getSessionKeys(sessionKey, sessionId) {
  return [sessionKey, sessionId].filter((value) => Boolean(value));
}
function storeSessionSnapshot(store, sessionKey, sessionId, messages) {
  for (const key of getSessionKeys(sessionKey, sessionId)) {
    store.set(key, messages);
  }
}
function readSessionSnapshot(store, sessionKey, sessionId) {
  for (const key of getSessionKeys(sessionKey, sessionId)) {
...
function clearSessionSnapshot(store, sessionKey, sessionId) {
  for (const key of getSessionKeys(sessionKey, sessionId)) {
    store.delete(key);
  }
}
...
function buildContextQuestion(prompt) {
  return [
...
function tokenize(value) {
  return normalizeForMatch(value).split(/[^a-z0-9]+/i).filter((token) => token.length >= 3);
}
function matchesForgetQuery(node, query, tokens) {
...
async function safeDiscover(api, logger) {
  try {
...
async function safeMaintain(api, logger) {
  try {
...
function register(api) {
  ...
  api.registerTool({
    name: "query_known",
    label: "Query Known",
    description: "Query Known for deep understanding about the user. Use when you need specific context about the user's patterns, preferences, relationships, or history.",
    parameters: {
      type: "object",
      ...
      async execute(_toolCallId, params) {
        ...
        try {
          return await withDatabase(api, async (db) => {
            const matches = db.getAllNodes().filter((node) => matchesForgetQuery(node, normalizedQuery, tokens)).sort((left, right) => right.confidence - left.confidence);
            if (matches.length === 0) {
...
${summary}`,
  ...
  api.registerService({
    ...
    start(ctx) {
      ...
      ctx.logger.info(
        `Known thinker started (discover every ${settings.discoverIntervalMs / 36e5}h, maintain every ${settings.maintainIntervalMs / 36e5}h).`
      );
    },
    stop(ctx) {
      if (discoverTimer) {
        clearInterval(discoverTimer);
        discoverTimer = void 0;
      }
      if (maintainTimer) {
        clearInterval(maintainTimer);
        maintainTimer = void 0;
      }
